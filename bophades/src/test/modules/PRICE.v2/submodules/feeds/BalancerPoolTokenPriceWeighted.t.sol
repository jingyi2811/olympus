...
contract BalancerPoolTokenPriceWeightedTest is Test {
    ...
    address  BALANCER_POOL = 0x96646936b91d6B9D7D0c47C496AfBF3D6ec7B6f8;
    uint256  BALANCER_POOL_TOTAL_SUPPLY = 44828046497101022591963;
    uint8  BALANCER_POOL_DECIMALS = 18;
    ...
    uint256  BALANCER_POOL_INVARIANT = 25432168041089078866395;

    uint256  USDC_BALANCE = 1112351166021;
    uint256  WETH_BALANCE = 581466708560997532338;

    uint256  USDC_PRICE = 1 * 1e18;
    uint256  WETH_PRICE = 1_917.25 * 1e18;
    ...
    function setUp() public {
        ...
        BALANCER_POOL_TOTAL_SUPPLY = pool.totalSupply();
        BALANCER_POOL_DECIMALS = pool.decimals();
        ...
        USDC_DECIMALS = IERC20(t[0]).decimals();
        WETH_DECIMALS = IERC20(t[1]).decimals();

        USDC_WEIGHT = pool.getNormalizedWeights()[0];
        WETH_WEIGHT = pool.getNormalizedWeights()[1];

        BALANCER_POOL_INVARIANT = pool.getInvariant();

        USDC_BALANCE = b[0];
        WETH_BALANCE = b[1];

        USDC_PRICE = 1 * 1e18;
        WETH_PRICE = 1_917.25 * 1e18;

        WETH_RATE =
        (((USDC_BALANCE * 1e18) / USDC_WEIGHT) * USDC_PRICE) /
        ((WETH_BALANCE * 1e18) / WETH_WEIGHT);

        USDC_RATE =
        (((WETH_BALANCE * 1e18) / WETH_WEIGHT) * WETH_PRICE) /
        ((USDC_BALANCE * 1e18) / USDC_WEIGHT);
    ...
    function _max(uint8 a, uint8 b) public view returns (uint8) {
        return a > b ? a : b;
    }

    function _min(uint8 a, uint8 b) public view returns (uint8) {
        return a < b ? a : b;
    }
    ...
    function test_getTokenPriceFromWeightedPool_tokenDecimalsFuzz() public {

        uint8 tokenDecimals_ = 7;
        uint8 tokenDecimals = uint8(bound(tokenDecimals_, MIN_DECIMALS, MAX_DECIMALS));
    ...
    function test_getTokenPriceFromWeightedPool_threeTokens() public {
        ...
        uint256 usdcRate = (((WETH_BALANCE * 1e18) / WETH_WEIGHT) * WETH_PRICE) /
            (((USDC_BALANCE / 2) * 1e18) / stablecoinWeight);
